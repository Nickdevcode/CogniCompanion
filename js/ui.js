...
(function () {
  ...
  /**
   * Injeta os ícones de redes sociais nos cards da equipe.
   *
   * O SVG mora aqui, e não no HTML, porque os dois paths de marca somam ~2 KB —
   * repeti-los em cada card encheria a página de ruído. Já o DESTINO mora no
   * HTML (`data-instagram` / `data-linkedin` no próprio card), que é onde a
   * pessoa está: assim mexer na equipe é mexer num lugar só.
   *
   * Um card sem a URL de uma rede simplesmente não ganha aquele ícone — melhor
   * do que o `href="#"` de antes, que parecia um link e não levava a lugar
   * nenhum. Os nós são construídos pela DOM API (sem innerHTML).
   */
  function setupSocials() {
    ...
    function buildLink(icon, url, nome) {
      const a = document.createElement("a");
      a.href = url;
      a.target = "_blank";
      // noopener fecha o acesso da aba nova ao window.opener; noreferrer evita
      // vazar de qual página o clique veio.
      a.rel = "noopener noreferrer";
      // Cinco cards com "Instagram" repetido não dizem nada a quem navega por
      // leitor de tela — o nome é o que diferencia um link do outro.
      a.setAttribute(
        "aria-label",
        nome ? icon.label + " de " + nome : icon.label
      );
    ...
    slots.forEach(function (slot) {
      const card = slot.closest(".team-card");
      const nomeEl = card && card.querySelector(".team-card__name");
      const nome = nomeEl ? nomeEl.textContent.trim() : "";

      const frag = document.createDocumentFragment();
      icons.forEach(function (icon) {
        const url = slot.getAttribute("data-" + icon.attr);
        if (!url) return; // sem perfil nessa rede: não desenha um link morto
        frag.appendChild(buildLink(icon, url, nome));
      });
