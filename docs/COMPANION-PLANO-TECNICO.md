...
> ⚠️ **Se um dia entrar "apagar os dados da minha filha" no painel** (e num TCC sobre criança a banca pode até pedir, por LGPD), **ele precisa nascer junto com o robô escutando DELETE no Realtime.** Hoje o servidor sobe pra nuvem, no boot, todo perfil que existe no disco dele e não existe no banco — é o que repovoou as 6 crianças em 26/ago/2026, e o preço é a **ressurreição**: criança apagada por fora do robô volta no boot seguinte. O trade-off é consciente e hoje é inofensivo, porque o site **não tem nenhum caminho de DELETE em `criancas`** (o único `.delete()` do site inteiro é num plano de estudo, no rollback de `criarPlanoComTarefas`). Sem essa linha, quem implementar a exclusão cria uma promessa quebrada sem nunca saber que criou.
...
> ⛔ **26/ago/2026 — O PAREAMENTO SAIU DO SERVIDOR LOCAL E FOI PRO BANCO.** O que está nos dois parágrafos abaixo descreve o caminho ANTIGO, que **não funciona mais a partir da Vercel**: desde o Chrome 141/142, um site de origem pública não alcança `127.0.0.1` nem a rede local sem permissão explícita do usuário (*Local Network Access*) — medido em Chrome 149, o `fetch` **pendura e nunca chega ao servidor**. Foi o que quebrou na apresentação do TCC. Os endpoints do robô **continuam existindo** e servem o painel localhost dele; o que mudou é quem o SITE chama:
>
> | O site chamava | O site chama agora | Observação |
> | --- | --- | --- |
> | `POST {SERVIDOR}/api/pareamento/vincular` | `rpc('vincular_por_codigo', { p_codigo })` | `SECURITY DEFINER`; o `responsavel_id` vem do `auth.uid()`, **não do corpo** |
> | `POST {SERVIDOR}/api/pareamento/desvincular` | `rpc('desvincular_crianca', { p_crianca_id })` | idempotente; só desvincula o que é do próprio pai |
> | `GET {SERVIDOR}/api/pareamento/codigo` | lê `criancas.codigo_pareamento` | o dado já vem no `select('*')` do boot — **zero rede** |
>
> Retorno da RPC: `{ok, jaPareado, criancaId, nome, idade, serie}` · `{ok:false, motivo}`, com `motivo` ∈ `codigo_invalido | ja_pareada | ja_tem_crianca | muitas_tentativas | sem_sessao`. Os quatro primeiros são os mesmos do servidor; `ja_tem_crianca` (trava single-child) e `muitas_tentativas` (rate limit, que o servidor dava por IP e a RPC precisa dar sozinha) são novos. `idade`/`serie` existem só pra a tela de sucesso **desambiguar homônimos** — há dois "Marcos" entre os perfis do robô, com códigos diferentes.
...
2. **Áudio é transcrito antes, não mandado pro modelo de chat.** `POST /v1/audio/transcriptions` com **`gpt-transcribe`** (US$ 0,0045/min), fallback pra `gpt-4o-mini-transcribe` se a conta não tiver o modelo. A transcrição entra no **mesmo pipeline de texto** — um caminho só, um schema só.
   > 🗓️ **23/set/2026 — o fallback mudou, e tem data pra morrer.** Era `whisper-1`; em 26/ago/2026 a OpenAI anunciou a saída de `whisper-1`, `gpt-4o-transcribe` e `gpt-4o-mini-transcribe` da API em **26/02/2027**. O fallback agora é o mesmo do robô (`STT_FALLBACK_MODEL`), e passada a data `transcrever()` simplesmente não o chama (`FALLBACK_SAI_DO_AR` em `api/_lib/openai.mjs`). O titular `gpt-transcribe` não está na lista. E o formato das dicas foi corrigido pro da doc: `languages[]` e `keywords[]` são **listas em multipart**, um campo por item — mandava-se `keywords` como UMA string com vírgulas, que a API aceitava como um termo só.
