...
| 📅 **"A semana" tinha três definições** (07/set) | O gráfico plotava 7 chaves de dia, a legenda colada nele filtrava `>= agora - 7 dias` (que varre parte de um **oitavo** dia) e o modo Mensal montava baldes de 6 dias e 1 instante andando de 7 em 7. Medido com 10 min/dia por 28 dias: o gráfico desenhava **70 min** e a frase embaixo dizia **1h 20min**; o Mensal perdia **40 dos 280 min** — e só quando a hora das conversas era diferente da hora em que o pai abria o painel, o que escondia o bug de qualquer teste redondo. Agora todos derivam de `janelaDeDias`/`chavesDeDias` (`format.js`): uma definição só |
| 🧮 **"Matérias exploradas" contava o nada** (07/set) | O contador era `new Set(conversas.map(c => c.materia)).size`, sem filtro: um `materia` nulo entrava como se fosse mais uma matéria, e um rótulo fora das 14 virava um balde que os cards de tempo descartavam — o tempo dele sumia da tela e o contador o somava. `materiaCanonica()` passou a ser a única resposta pra "que matéria é esta", nos cards e nos dois contadores (Início e Aprendizado) |
| 🫥 **Vazio e quebrado tinham a mesma cara** (07/set) | A Dica e o Bilhete caem em silêncio da fonte fresca (o robô) pra estável (a tabela) e daí pro texto padrão — de propósito, porque robô desligado não é erro. O preço era que "escrito agora", "escrito há três dias" e "nunca escrito" chegavam **idênticos** à tela, e a pergunta que sobrava era sempre *isso está funcionando?*. Ganharam uma linha discreta (`Escrito há 2 dias. Atualiza quando o robô estiver ligado.`) e um `console.info` afirmativo dizendo de qual degrau veio o texto |
| 🌙 **O "hoje" da IA era o de Londres** (25/set) | O painel mandava pro `/api/plano-de-material` o dia em UTC (`toISOString()`), que no Brasil já é amanhã das 21h à meia-noite: justo a hora da mochila. "Entregar amanhã" digitado às 22h virava depois de amanhã, e "entregar terça" numa segunda à noite podia cair na terça da semana seguinte. Agora vai o dia do aparelho (`dayKey`), e a função, quando não recebe data, usa o dia de Brasília. A mensagem da cota também parou de prometer "amanhã": a janela é de 24 horas corridas |
| 📡 **Sem rede virava "primeira visita"** (25/set) | Se a leitura da criança falhasse (Wi-Fi piscando), o `main.js` tratava igual a "não há criança" e jogava um pai pareado no "Passo 1 de 3 · Boas-vindas". Agora a tela diz que não conseguiu carregar e oferece **Tentar de novo**. O mesmo botão entrou no erro genérico das seções, que era um beco sem saída |
| 🔎 **A busca do Diário exigia acento** (25/set) | "fracao" não achava "fração", e o assunto da conversa (`topico`, o nome do chip que o pai vê em Aprendizado) nem era procurado. Agora a busca ignora acento e olha o assunto também. De brinde, `Esc` fecha o menu de matéria. E como o robô passou a gravar o assunto **com** acento nesse mesmo dia, "fracoes" (antigo) e "frações" (novo) viravam dois chips em Aprendizado e no bilhete: `topicosUnicos()` (`format.js`) junta os dois e mostra a grafia acentuada |
| 🎂 **Idade 45 passava** (25/set) | O campo tinha `min`/`max`, mas o formulário tem `novalidate`: a idade do pai digitada no lugar errado ia pro banco e a Cogni calibrava a conversa pra um adulto. Fora de 4 a 18 (a faixa que o robô também aceita), o formulário para e diz por quê |
| ✏️ **Nada de gabarito no card** (25/set) | Os dois prompts (`prompt.mjs` e o ✨ do detalhe em `melhorar.mjs`) agora mandam copiar as contas e as perguntas, mas **nunca** o resultado: a Cogni ensina a chegar na resposta, e a resposta escrita no card vira cola. Conferido ao vivo: as contas continuam no card, os resultados não aparecem |
...
| `js/dashboard/tour-passos.js` | **O roteiro** do tutorial (só conteúdo: título, texto e âncora de cada uma das 9 paradas) |
...
| `js/dashboard/recado-frescor.js` | A linha **"de quando é este recado"** embaixo da Dica e do Bilhete, e o `console.info` que diz de qual fonte o texto veio |
